...
createVis = function() {
    ...
    /*
    var movieCircles = vis.selectAll(".movie")
        .data(movieArr[displayIdx].movies)
    ...
    console.log('movieArr[displayIdx].movies',movieArr[displayIdx].movies);

    var movieLabels = vis.selectAll(".movielabel")
            .data(movieArr[displayIdx].movies)
            .enter()
            .append("svg:text")
            .classed("movielabel",true)
            .attr("x", function(d) { console.log(d); return xScale(d.week);})
            .attr("y", function(d) { return yScale(d.weeklyGross);})
            .attr("dx", function(d) { return circleSize.labelMoveRight + rScale(d.likes) ;})
            .attr("dy",  function(d) { return circleSize.labelMoveDown;})
            .attr("text-anchor","end")
...
*/
    /*
              ...
              var populations = svg.selectAll(".population")
                    .data(dataSetFull)
                    .enter().append("g")
              ...
              var line = d3.svg.line()
                    ...
                    .y(function(d) { return (yScale(d.pop) + bbVis.y); });

              populations.append("path")
                    .attr("class", "line")
                    .attr("d", function(d) {
                        return line(d.popArr); })
                    .style("stroke", function(d) { return color(d.pcat); });

              for (var key in dataSetFull) {
                    var realPoints = svg.selectAll(".realpoint")
                        .data(dataSetFull[key].popArr.filter (
                            function (d,i) {
                                 return (dataSetFull[key].isRealDataArr[i] == 'Y');
                            }))
                        .enter().append("svg:circle")
                        .attr("stroke", "blue")
                        .attr("fill", function(d, i) { return "blue" })
                        .attr("cx", function(d, i) { return (xScale(d.year) + bbVis.x) })
                        .attr("cy", function(d, i) { return (yScale(d.pop) + bbVis.y) })
                        .attr("r", function(d, i) { return 2 });

                    var estPoints = svg.selectAll(".realpoint")
                        .data(dataSetFull[key].popArr.filter (
                            function (d,i) {
                                return (
                                    (dataSetFull[key].isRealDataArr[i] == 'N')
                                    && ((dataSetFull[key].popArr[i].year % 20 == 0))  //only display estimates for every 20 years
                                );
                            }))
                        .enter().append("svg:circle")
                        .attr("stroke", "grey")
